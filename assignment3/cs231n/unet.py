...
class Unet(nn.Module):
    ...
    def cfg_forward(self, x, time, model_kwargs={}):
        ...
        ##################################################################
        # Conditional prediction: eps(x_t, cond)
        cond_kwargs = copy.deepcopy(model_kwargs)
        eps_cond = self.forward(x, time, cond_kwargs)

        # Unconditional prediction: eps(x_t, empty)
        uncond_kwargs = copy.deepcopy(model_kwargs)
        uncond_kwargs["text_emb"] = None
        eps_uncond = self.forward(x, time, uncond_kwargs)

        # CFG: (scale + 1) * eps_cond - scale * eps_uncond
    ...
    def forward(self, x, time, model_kwargs={}):
        ...
        for up_block in self.ups:
            upsample, resnet1, resnet2 = up_block
            x = upsample(x)
