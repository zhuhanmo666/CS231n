...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        denom_1 = sum(torch.exp(sim(z_k, out[i]) / tau) for i in range(2 * N) if i != k)
        loss_k = -torch.log(numerator_1 / denom_1)

        # Compute l(k+N, k)
        numerator_2 = torch.exp(sim(z_k_N, z_k) / tau)
        denom_2 = sum(torch.exp(sim(z_k_N, out[i]) / tau) for i in range(2 * N) if i != k + N)
        loss_k_N = -torch.log(numerator_2 / denom_2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # Element-wise dot product, sum along D dimension -> [N, 1]
    pos_pairs = (left_norm * right_norm).sum(dim=1, keepdim=True)
...
def compute_sim_matrix(out):
    ...
    # Matrix multiply to get all pairwise dot products -> [2N, 2N]
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    pos_pairs = torch.cat([pos_pairs, pos_pairs], dim=0)  # [2*N, 1]
