...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        model_out = self.model(x_t, t_tensor, model_kwargs)

        # Step 2: Recover x_start and noise depending on objective
        if self.objective == "pred_noise":
            pred_noise = model_out
            x_start = self.predict_start_from_noise(x_t, t_tensor, pred_noise)
        elif self.objective == "pred_x_start":
            x_start = model_out
            pred_noise = self.predict_noise_from_start(x_t, t_tensor, x_start)

        # Step 3: Clamp x_start to valid range
        x_start = x_start.clamp(-1.0, 1.0)

        # Step 4: Get posterior mean and std, then sample x_{t-1}
        posterior_mean, posterior_std = self.q_posterior(x_start, x_t, t_tensor)

        # Only add noise when t > 0
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        ####################################################################
        # Step 1: Sample x_t from q(x_t | x_0)
        x_t = self.q_sample(x_start, t, noise)

        # Step 2: Get model predictions
        model_out = self.model(x_t, t, model_kwargs)

        # Step 3: Weighted MSE loss
        loss = (loss_weight * ((model_out - target) ** 2)).mean()
        ####################################################################
